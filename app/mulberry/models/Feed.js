dojo.provide('mulberry.models.Feed');
 ...
 */
dojo.declare('mulberry.models.Feed', [], {
  ...
  _onLoad : function(dfd, data) {
    ...
    if (data && data.query && data.query.results && data.query.results.item) {
      this.items = dojo.map(data.query.results.item, function(item, index) {
        item.index = index;
        return new mulberry.models.FeedItem(item, this);
      }, this);
 ...
 */
dojo.declare('mulberry.models.FeedItem', null, {
  /**
   * @constructor
   */
  constructor : function(item, feed) {
    ...
    dojo.mixin(this, {
      ...
      url : mulberry.app.URL.feedItem(feed.id, item.index),
      link : item.link,
