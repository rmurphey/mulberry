dojo.provide('mulberry.models._StorableAsset');

dojo.require('mulberry.app.URL');

(function() {

var appUrl = mulberry.app.URL.storedAsset;

dojo.declare('mulberry.models._StorableAsset', [], {
  ...
  _isStreamed : function(item, type) {
    ...
    if (!mulberry.app.manifest || this.store.getValue(item, 'streamed')) { return true; }
    ...
    // next, we figure out where to look in mulberry.app.manifest
    var lookup = mulberry.app.manifest[this.store.getValue(item, 'type') + 's'];
