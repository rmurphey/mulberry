...
module Builder
  class JavaScript < Builder::TaskBase
    COPYRIGHT_FILE = File.join(
      Mulberry::TouraAPP::Directories.root,
      "profiles",
      "copyright.txt"
    )
    ...
    def initialize(build, task_name)
      ...
      @location = Mulberry::TouraAPP::Directories.javascript
      @client_dir = nil
    ...
    def get_dojo
      ...
      dest = File.join(Mulberry::TouraAPP::Directories.javascript, "dojo-release-#{ver}-src")

      unless File.exists?(dest)
        Dir.chdir Mulberry::TouraAPP::Directories.javascript
        ...
        dojo_installed = system %{curl --insecure -o - #{dojo} | tar -C #{Mulberry::TouraAPP::Directories.javascript} -xzf -}
    ...
    def base_profile
      ...
      if @build.build_helper.respond_to? 'custom_js_source'
        ...
        if custom_js_source
          @client_dir = File.join(Mulberry::TouraAPP::Directories.root, 'javascript', 'client_tmp')
