describe("favorites API", function() {
  var api, data, ds, node, id = 'node-368';
  ...
  describe("favorites access", function() {
    ...
    it("should return a list of saved favorites", function() {
      ...
      dojo.publish('/favorite/add', [ dataAPI.getModel('node-368') ]);
      dojo.publish('/favorite/add', [ dataAPI.getModel('node-369') ]);
    ...
    it("should indicate whether an object is a favorite", function() {
      ...
      expect(api.isFavorite(dataAPI.getModel('node-370'))).toBeFalsy();
    });
  });
